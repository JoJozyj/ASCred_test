...
std::future<void> process_mu_hashes_async(int i, int j, const unsigned char* messagedigest,
                                           user_state* state, unsigned char* rand) {
    return std::async(std::launch::async, [i, j, messagedigest, state, rand]() {
        for (int l = 0; l < PAR_L; ++l) {
            ...
            hash_mu(messagedigest, &state->varphis[idx_r * SECPAR],
                    &rand[i * (PAR_L + 1) * SECPAR + l * SECPAR]);
        }
    });
}
...
std::future<void> process_c_and_mu_async(int i, int j, unsigned char* rand, user_state* state, G1& g1alpha) {
    return std::async(std::launch::async, [i, j, rand, state, &g1alpha]() {
        for (int c = 0; c < PAR_L; ++c) {
            ...
            hash_bls(&rand[i * (PAR_L + 1) * SECPAR + c * SECPAR], state->info, state->mu_hashes[idx_c]);
            G1::mul(g1alpha, state->ctx->g1, state->alphas[idx_c]);
...
void user_challenge(const context *ctx, const publickey *pk,
    ...
    for (int i = 0; i < PAR_K; ++i) {
        for (int j = 0; j < PAR_N; ++j) {
            // 异步处理 mu hashes
            futures.push_back(process_mu_hashes_async(i, j, messagedigest, state, rand));
...
void user_challenge_not_optimize(const context *ctx, const publickey *pk,
		const unsigned char *info, const unsigned char *messagedigest,
		user_state *state, challenge *chall) {

	// save pk and ctx for later
	std::cout << "start user_chanllenge 111 " << std::endl;
	std::cout << "info1 " << info <<std::endl;
	state->pk = pk;
	state->ctx = ctx;
	...
	for (int i = 0; i < PAR_K; ++i) {
		for (int j = 0; j < PAR_N; ++j) {
			...
			for(int l = 0;l < PAR_L; ++l){
				...
				std::cout << "idx_r_hash_mu_rand " <<idx * (PAR_L+ 1)+l<<  std::endl;
			}
			//sample gamma_{i,j} next to it
			random_bytes(&rand[ (idx+1)*(PAR_L+1) * SECPAR - SECPAR], SECPAR);
			//std::cout << "idx_r_random_bytes_rand " <<(idx+1)*(PAR_L+1)-1<<  std::endl;
			// compute com_{i,j}
			hash_r(&rand[idx * (PAR_L + 1) * SECPAR], PAR_L , &com[idx * SECPAR]);
			//std::cout << "idx_r_hash_r_rand " <<idx * (PAR_L + 1)<<  std::endl;
			...
			for(int c = 0;c < PAR_L;++c){
				...
				//std::cout << "idx_r_hash_bls_rand " <<idx * (PAR_L+ 1)  + c<<  std::endl;
	...
	for (int i = 0; i < PAR_K; ++i) {
		...
		for(int l = 0; l < PAR_L; ++l){
			//int idx_c = (i * PAR_N  + Ji) * PAR_L;
			chall->c[i][l] = state->c[idx_c + l];
			std::cout << "idx_c_chall_ji " << idx_c <<  std::endl;
		}
		...
		memcpy(&chall->rand[i * (PAR_N - 1) * (PAR_L + 1) * SECPAR],
				&rand[i * PAR_N * (PAR_L + 1) * SECPAR], Ji * (PAR_L + 1) * SECPAR);
				//std::cout << "rand to chall_rand 1 " << i * PAR_N * (PAR_L + 1) << std::endl;
		memcpy(&chall->rand[i * (PAR_N - 1) * (PAR_L + 1) * SECPAR + Ji * (PAR_L + 1) * SECPAR],
				&rand[(i * PAR_N + Ji + 1) * (PAR_L + 1) * SECPAR],
				(PAR_N - 1 - Ji) * (PAR_L + 1) * SECPAR);
				//std::cout << "rand to chall_rand 2 " << (i * PAR_N + Ji + 1) * (PAR_L + 1) <<std::endl;
	}
	std::cout << "info4 " << info <<std::endl;
	std::cout << "second loop over " << std::endl;

}


bool user_finalize_not_optimize(user_state *state, challenge *chall, response *resp,
		signature &sig) {
